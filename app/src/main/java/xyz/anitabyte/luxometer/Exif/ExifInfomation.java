...
package xyz.anitabyte.luxometer.Exif;
	...
	public boolean readImage(byte data[], boolean read_thumbnail) throws IOException{
		...
		if(byte1 == 0xff && byte2 == 0xd8){// jpeg commpress
			...
			if (byte1 == 0xff && byte2 == 0xe0) {
				//we have a JFIF header. Let's skip it and get to EXIF
				//the next two bytes are the length
				byte1 = memstream.read();
				byte2 = memstream.read();
				int length = byte1 + byte2;
				Log.v("LENGTHOFJFIF", Integer.toString(length));
				memstream.readNBytes(length - 2); //the two length bytes are included in length
			}
			byte1 = memstream.read();
			byte2 = memstream.read();
			Log.v("BYTESPOSTJFIF", Integer.toHexString(byte1) + Integer.toHexString(byte2));
			if ( byte1 == 0xff && byte2 == 0xe1 ){ // APP1
				...
				if(!str.equals("Exif")){// exit if it isn't Exif
					Log.v("EXIF", "No EXIF found");
					return false;
				}
				else {
					Log.w("EXIF", "Found EXIF");
				}
				while(true){
					byte1 = memstream.read();
					if(byte1 != '\0') break;
